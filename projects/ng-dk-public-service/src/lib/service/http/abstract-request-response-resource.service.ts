...
> {
  ...
  public list(params?: {}): Observable<COLLECTION_RESPONSE_DTO> {
    return this.httpClient.get<COLLECTION_RESPONSE_DTO>(`${this.getBaseUri()}`, { params });
  }

  public listForUser(userUuid: string, params? : {}): Observable<COLLECTION_RESPONSE_DTO> {
    return this.httpClient.get<COLLECTION_RESPONSE_DTO>(
      `/v6/${ResourceMapping.USERS}/${userUuid}/${this.endpoint}`,
      { params }
    );
  }
